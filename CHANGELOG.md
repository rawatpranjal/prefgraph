...
- Money Pump Index objective. Both backends computed Karp's minimum cycle mean, which divides a cycle's money-pump savings by its edge count, instead of the Money Pump Index, which is the minimum cost-to-budget cycle ratio (Smeulders and Spieksma 2013 Theorem 2; Megiddo 1979). On heterogeneous-budget cycles the reported value could exceed the true maximum money-pump fraction. Both backends now compute the exact minimum cost-to-budget ratio and agree with a brute-force oracle to about 1e-10. The MPI backend-parity test, previously expected-to-fail for this divergence, is now a tight passing assertion.
- Houtman-Maks over observations. The menu Houtman-Maks counted items not in cycles with an ad-hoc heuristic in Rust and returned an invalid subset (one that still violated SARP) in the pure-Python path. Both backends now compute the exact largest subset of observations whose induced item graph is acyclic, via a ranking ILP (Houtman and Maks 1985; Demuynck and Rehbeck 2023 Definition 3), and match an exhaustive oracle exactly. The budget greedy, which over-removed badly (at T=100 it removed 97 observations where the exact ILP removes 3), is now used only above T=100 and documented as a loose upper bound rather than an estimate.
- Pure-Python fallback no longer reports confident placeholder values. Without the Rust extension the Engine left many fields at misleading defaults (`is_warp_la` False flagged every user as a violator, `max_scc` 0 was structurally impossible, network and SCC statistics read as zero). The fallback now computes every one of these from the relations it already builds, mirroring the Rust formulas, so the two backends agree to floating-point precision. The backend-parity suite grew from nine tests to twenty-nine, adding SCC, network, VEI, and menu parity classes.
- Quasilinearity test no longer truncates. `check_quasilinearity` enumerated cycles only up to length three, so a violation that first appeared in a longer cycle was silently reported as quasilinear. The verdict is now always exhaustive (all cycle lengths, Bellman-Ford); the length parameter only affects the human-readable breakdown.
- VEI labeling corrected. `compute_vei`'s docstring claimed Varian's exact index with an objective (minimise the sum of one minus the efficiencies) that contradicted the code (which minimises the sum of the efficiencies). It actually computes a per-observation efficiency lower bound, a fast approximation. The docstring now says so and points to `vei_exact` (the exact Mononen 2023 formulation in Rust) for the exact index.
...
- Remaining limitation, now documented rather than silent. Without the Rust backend `vei_exact` falls back to the VEI relaxation, a lower bound on the exact value, because a native Python exact-VEI (Mononen) solver is not yet implemented.
