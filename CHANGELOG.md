...
- Correctness findings surfaced during this audit are parked for follow-up reconciliation. The pure-Python `vei_exact` uses the VEI relaxation rather than the exact LP that Rust uses, pending the VEI fix, and the quasilinear test truncates its cycle search at length three. See the parked findings in `docs/roadmap.md`.
