...
- **Version bumps are the smallest honest increment. Never inflate a version to signal effort.** Patch for fixes, even ones that change reported numbers. Minor only for new API surface. Wrong is shipping a correctness fix as 0.7.0 to make it look bigger. Right is 0.6.2 with a plain changelog line.
