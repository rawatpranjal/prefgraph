References
==========

This page provides a comprehensive bibliography of the academic literature underlying PrefGraph,
organized by topic.

.. note::

   The core reference for PrefGraph is Chambers & Echenique (2016), *Revealed Preference Theory*.
   Chapter numbers throughout the documentation refer to this textbook.


Selected Reading
----------------

Recent applied work using revealed preference methods on real-world data:

- **Choi et al. (2014)** - Lab CCEI benchmarks (≈0.88). *American Economic Review* (experiment).
- "Revealed preference theory: An algorithmic outlook" (2019). *European Journal of Operational Research*.
- **Deb, Kitamura, Quah & Stoye (2023)** - Revealed price preference: Theory and empirical analysis. *Review of Economic Studies*, 90(2), 707–743.
- "The Moral Mind(s) of Large Language Models" (arXiv, 2024/2025).
- **Devine et al. (2025)** - Context effects in economic choice: A meta-analysis. *Working paper*.
- **GARP‑EFM (2026)** - "GARP‑EFM: Improving Foundation Models with Revealed Preference Structure." *arXiv* (March 2026).


Foundational Works
------------------

**Core Textbook:**

- Chambers, C. P., & Echenique, F. (2016). *Revealed Preference Theory*. Cambridge University Press.
  `[Amazon] <https://www.amazon.com/Revealed-Preference-Econometric-Society-Monographs/dp/1107087805>`__

**Classical Foundations:**

- Afriat, S. N. (1967). The construction of utility functions from expenditure data.
  *International Economic Review*, 8(1), 67-77.
  `[Paper] <https://www.jstor.org/stable/2525382>`__

- Samuelson, P. A. (1938). A note on the pure theory of consumer's behaviour.
  *Economica*, 5(17), 61-71.
  `[Paper] <https://www.jstor.org/stable/2548836>`__

- Richter, M. K. (1966). Revealed preference theory.
  *Econometrica*, 34(3), 635-645.
  `[Paper] <https://www.jstor.org/stable/1909773>`__

- Hicks, J. R. (1939). *Value and Capital*. Oxford University Press.


Budget-Based Revealed Preference
--------------------------------

**GARP and Consistency Testing:**

- Varian, H. R. (1982). The nonparametric approach to demand analysis.
  *Econometrica*, 50(4), 945-973.
  `[Paper] <https://www.jstor.org/stable/1912771>`__

- Varian, H. R. (1983). Non-parametric tests of consumer behaviour.
  *Review of Economic Studies*, 50(1), 99-110.
  `[Paper] <https://www.jstor.org/stable/2296957>`__

**Efficiency Measures:**

- Afriat, S. N. (1972). Efficiency estimation of production functions.
  *International Economic Review*, 13(3), 568-598.

- Varian, H. R. (1990). Goodness-of-fit in optimizing models.
  *Journal of Econometrics*, 46(1-2), 125-140.
  `[DOI] <https://doi.org/10.1016/0304-4076(90)90051-T>`__

- Houtman, M., & Maks, J. A. H. (1985). Determining all maximal data subsets consistent with revealed preference.
  *Kwantitatieve Methoden*, 19, 89-104.

- Echenique, F., Lee, S., & Shum, M. (2011). The Money Pump as a measure of revealed preference violations.
  *Journal of Political Economy*, 119(6), 1201-1223.
  `[Paper] <https://www.jstor.org/stable/10.1086/665011>`__

**Test Power:**

- Bronars, S. G. (1987). The power of nonparametric tests of preference maximization.
  *Econometrica*, 55(3), 693-698.
  `[Paper] <https://www.jstor.org/stable/1913608>`__

- Andreoni, J., Gillen, B. J., & Harbaugh, W. T. (2013). The power of revealed preference tests:
  Ex-post evaluation of experimental design.
  Working paper.


Menu-Based Choice Theory
------------------------

**Abstract Choice:**

- Arrow, K. J. (1959). Rational choice functions and orderings.
  *Economica*, 26(102), 121-127.

- Sen, A. K. (1971). Choice functions and revealed preference.
  *Review of Economic Studies*, 38(3), 307-317.

**Experimental Evidence:**

- Choi, S., Kariv, S., Muller, W., & Silverman, D. (2014). Who is (more) rational?
  *American Economic Review*, 104(6), 1518-1550.
  `[DOI] <https://doi.org/10.1257/aer.104.6.1518>`__


Limited Attention and Consideration Sets
----------------------------------------

**Deterministic Attention:**

- Masatlioglu, Y., Nakajima, D., & Ozbay, E. Y. (2012). Revealed Attention.
  *American Economic Review*, 102(5), 2183-2205.
  `[DOI] <https://doi.org/10.1257/aer.102.5.2183>`__

  *Key contribution: Introduces WARP(LA), characterizing choice with limited attention via
  attention filters. Shows that WARP violations can be rationalized by assuming consumers
  don't see all options.*

- Lleras, J. S., Masatlioglu, Y., Nakajima, D., & Ozbay, E. Y. (2017).
  When more is less: Limited consideration.
  *Journal of Economic Theory*, 170, 70-85.
  `[DOI] <https://doi.org/10.1016/j.jet.2017.04.004>`__

**Stochastic Consideration:**

- Manzini, P., & Mariotti, M. (2014). Stochastic choice and consideration sets.
  *Econometrica*, 82(3), 1153-1176.
  `[DOI] <https://doi.org/10.3982/ECTA10575>`__

**Random Attention Model:**

- Cattaneo, M. D., Ma, X., Masatlioglu, Y., & Suleymanov, E. (2020).
  A Random Attention Model.
  *Journal of Political Economy*, 128(7), 2796-2836.
  `[DOI] <https://doi.org/10.1086/706861>`__

  *Key contribution: Extends attention models to stochastic choice. Provides testable
  restrictions and bounds on attention probabilities from choice frequency data.*


Stochastic Choice and Random Utility
------------------------------------

**Foundational Theory:**

- Block, H. D., & Marschak, J. (1960). Random orderings and stochastic theories of responses.
  In I. Olkin et al. (Eds.), *Contributions to Probability and Statistics* (pp. 97-132).
  Stanford University Press.

- McFadden, D. (1974). Conditional logit analysis of qualitative choice behavior.
  In P. Zarembka (Ed.), *Frontiers in Econometrics* (pp. 105-142). Academic Press.

- Luce, R. D. (1959). *Individual Choice Behavior: A Theoretical Analysis*. Wiley.

**Modern Developments:**

- Kitamura, Y., & Stoye, J. (2018). Nonparametric analysis of random utility models.
  *Econometrica*, 86(6), 1883-1909.
  `[DOI] <https://doi.org/10.3982/ECTA14478>`__

- Smeulders, B., Crama, Y., & Spieksma, F. C. R. (2021).
  Revealed preference tests of collectively rational consumption behavior:
  Formulations and algorithms.
  *Operations Research*, 69(4), 1237-1253.
  `[DOI] <https://doi.org/10.1287/opre.2019.1920>`__

**IIA and Regularity:**

- Debreu, G. (1960). Review of R. D. Luce, Individual choice behavior.
  *American Economic Review*, 50, 186-188.

- Fudenberg, D., Iijima, R., & Strzalecki, T. (2015). Stochastic choice and revealed perturbed utility.
  *Econometrica*, 83(6), 2371-2409.
  `[DOI] <https://doi.org/10.3982/ECTA12660>`__


Integrability and Demand Analysis
---------------------------------

**Slutsky Conditions:**

- Hurwicz, L., & Uzawa, H. (1971). On the integrability of demand functions.
  In J. S. Chipman et al. (Eds.), *Preferences, Utility, and Demand* (pp. 114-148).
  Harcourt Brace Jovanovich.

- Chiappori, P. A., & Rochet, J. C. (1987). Revealed preferences and differentiable demand.
  *Econometrica*, 55(3), 687-691.
  `[Paper] <https://www.jstor.org/stable/1913607>`__

**Nonparametric Bounds:**

- Blundell, R., Browning, M., & Crawford, I. (2008). Best nonparametric bounds on demand responses.
  *Econometrica*, 76(6), 1227-1262.
  `[DOI] <https://doi.org/10.3982/ECTA6069>`__


Welfare Analysis
----------------

**Compensating and Equivalent Variation:**

- Vartia, Y. O. (1983). Efficient methods of measuring welfare change and compensated income
  in terms of ordinary demand functions.
  *Econometrica*, 51(1), 79-98.
  `[Paper] <https://www.jstor.org/stable/1912249>`__

**Welfare Bounds:**

- Apesteguia, J., & Ballester, M. A. (2015). A measure of rationality and welfare.
  *Journal of Political Economy*, 123(6), 1278-1310.
  `[DOI] <https://doi.org/10.1086/683838>`__

- Deb, R., Kitamura, Y., Quah, J. K. H., & Stoye, J. (2023).
  Revealed price preference: Theory and empirical analysis.
  *Review of Economic Studies*, 90(2), 707-743.
  `[DOI] <https://doi.org/10.1093/restud/rdac041>`__


Production Theory
-----------------

- Varian, H. R. (1984). The nonparametric approach to production analysis.
  *Econometrica*, 52(3), 579-597.
  `[Paper] <https://www.jstor.org/stable/1913466>`__


Context Effects
---------------

**Decoy Effect:**

- Huber, J., Payne, J. W., & Puto, C. (1982). Adding asymmetrically dominated alternatives:
  Violations of regularity and the similarity hypothesis.
  *Journal of Consumer Research*, 9(1), 90-98.
  `[Paper] <https://www.jstor.org/stable/2488940>`__

- Devine, S., et al. (2025). Context effects in economic choice: A meta-analysis.
  Working paper.

**Compromise Effect:**

- Simonson, I. (1989). Choice based on reasons: The case of attraction and compromise effects.
  *Journal of Consumer Research*, 16(2), 158-174.

- Kivetz, R., Netzer, O., & Srinivasan, V. (2004). Alternative models for capturing the compromise effect.
  *Journal of Marketing Research*, 41(3), 237-257.
  `[DOI] <https://doi.org/10.1509/jmkr.41.3.237.35990>`__

**Platform Applications:**

- Seshadri, A., Peysakhovich, A., & Ugander, J. (2019). Discovering context effects from raw choice data.
  *Proceedings of ICML 2019*.


Algorithmic Methods
-------------------

**Computational Approaches:**

- Smeulders, B., Crama, Y., & Spieksma, F. C. R. (2019). Goodness-of-fit measures for revealed
  preference tests: Complexity results and algorithms.
  *ACM Transactions on Economics and Computation*, 7(1), 1-26.
  `[DOI] <https://doi.org/10.1145/3290952>`__

- Demuynck, T., & Rehbeck, J. (2023). Computing revealed preference goodness-of-fit measures
  with integer programming.
  *Economic Theory*, 75, 1101-1130.
  `[DOI] <https://doi.org/10.1007/s00199-022-01431-7>`__


Surveys and Reviews
-------------------

- Dziewulski, P., Lanier, J., & Quah, J. K. H. (2024). Revealed preference: A survey.
  Working paper.

- Hoderlein, S., & Stoye, J. (2014). Revealed preferences in a heterogeneous population.
  *Review of Economics and Statistics*, 96(2), 197-213.
  `[DOI] <https://doi.org/10.1162/REST_a_00394>`__


Time Inconsistency
------------------

- Strack, P., & Taubinsky, D. (2021). Dynamic preference "reversals" and time inconsistency.
  Working paper.


Paired Comparisons
------------------

- Cattelan, M. (2012). Models for paired comparison data: A review with emphasis on dependent data.
  *Statistical Science*, 27(3), 412-433.
  `[DOI] <https://doi.org/10.1214/12-STS396>`__

- Benson, A., Kumar, R., & Tomkins, A. (2016). On the relevance of irrelevant alternatives.
  *Proceedings of WWW 2016*.


See Also
--------

- :doc:`theory_landscape` - Mathematical foundations
- :doc:`budget/index` - Step-by-step guides
- :doc:`api` - API documentation
